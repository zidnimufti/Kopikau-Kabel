// --- FILE: src/pages/barista/BaristaPage.tsx (QUEUE TANPA ORDER ID, TANPA HAPUS) ---
...
import {
  getActiveMenu,
  createOrder,
  getPendingOrdersWithItems, // ambil pending orders + items
  updateOrderStatus,
  getOrderWithItems,         // untuk edit
  updateOrderAndItems,       // untuk edit
} from '../../api/orderApi';
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {isEditing && onCancelEdit ? (
            <button onClick={onCancelEdit} className="text-sm px-3 py-1 rounded bg-gray-100">Batalkan Edit</button>
          ) : null}
          {showClose ? (
            <button onClick={onClose} className="md:hidden text-sm px-3 py-1 rounded bg-gray-100">Tutup</button>
          ) : null}
        </div>
      </div>
      ...
      <div key={order.id} className="bg-yellow-100 p-3 rounded-md shadow flex flex-col">
        ...
        <ul className="mt-3 text-sm text-gray-800 space-y-1">
          ...
          {(!order.items || order.items.length === 0) && (
            <li className="text-gray-500">Belum ada item.</li>
          )}
        </ul>

        {/* aksi: Edit / Selesai / Batal */}
        <div className="flex gap-2 mt-3">
          <button onClick={() => onEdit(order.id)} className="flex-1 bg-blue-500 text-white text-xs py-1 rounded hover:bg-blue-600">
            Edit
          </button>
          <button onClick={() => onUpdateStatus(order.id, 'completed')} className="flex-1 bg-green-500 text-white text-xs py-1 rounded hover:bg-green-600">
            Selesai
          </button>
          <button onClick={() => onUpdateStatus(order.id, 'cancelled')} className="flex-1 bg-red-500 text-white text-xs py-1 rounded hover:bg-red-600">
            Batal
          </button>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col h-screen min-h-0 bg-gray-50">
      ...
      <div className="md:hidden sticky bottom-0 left-0 right-0 bg-white border-t p-3">
        <div className="flex items-center gap-2">
          <button onClick={() => setIsCartOpen(true)} className="flex-1 bg-indigo-600 text-white py-3 rounded-md font-semibold">
            {editingOrderId ? 'Edit Order' : 'Cart'} ({totalItems}) • {new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(totalPrice)}
          </button>
        ...
        <div className="fixed inset-0 z-40">
          ...
          <div className="absolute inset-x-0 md:inset-auto md:right-6 md:left-6 top-10 bottom-10 bg-white rounded-xl shadow-xl p-4 overflow-y-auto">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setIsQueueOpen(false)} className="px-3 py-1 rounded bg-gray-100">Tutup</button>
            </div>
            <PendingOrdersList
